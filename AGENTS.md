...
- 현재 구현 범위는 서비스 정의·등록 상태 전이, `LogicalVersion`·snapshot `LogicalClock` 기반 immutable snapshot·결정적 sync 병합, 단일 mutation gate·복구 coordinator, canonical 저장 XML v1·`directory.xml`·`pending.xml`·`config.xml`·DPAPI `peer.dat` store와 다중 파일 복구 저널, 시스템 파일 로그·보존과 보안 진단 primitive다. External 세 endpoint, `WDOG` loopback health와 Admin 12개 endpoint를 실제 메인 `HttpListener` host에 연결했고 endpoint별 인증 선택·deadline·drain, Admin application handler, Peer pairing·session·인증 HTTP exchange·scheduler 소스가 있다. Service·Watchdog·Tray 프로세스 최선두에는 현재 디렉터리를 제외하는 fail-closed native DLL 검색 정책을 적용했다. Watchdog Windows Service·Named Pipe, Tray 라이트 테마 UI·Admin·와치독 client, x64 MSTest 프로젝트와 `tools/test.ps1`, Inno Setup 소스와 `tools/package.ps1`도 있다.
- 현재 남은 검증 범위는 현재 작업 트리의 locked restore·Debug/Release 빌드·테스트·package, 실제 Windows Service·`HttpListener`·Negotiate/NTLM·Event Log·DPAPI·ACL·방화벽 실행, Inno 설치·repair·upgrade·rollback·uninstall과 지원 OS·AD·Workgroup 조합이다. Peer 인증 전 endpoint-only 제한과 release·revoke의 별도 수치 정책은 상세 명세에서 확정하기 전 임의로 추가하지 않는다.
...
- installer는 데이터 루트와 기존 메인·와치독 서비스 및 제품 등록이 모두 없던 진짜 최초 설치에서만 canonical empty `directory.xml`·`pending.xml`과 최초 `config.xml`을 만든다. 보존 데이터가 있는 재설치·repair에서는 누락된 상태 파일을 빈 파일로 재생성하지 않는다. runtime도 active journal 없는 두 state primary의 동시 누락을 빈 snapshot으로 해석하지 않고 fail closed한다.
