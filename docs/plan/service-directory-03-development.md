...
현재 저장소에는 x64 솔루션과 논리 버전 snapshot·결정적 sync 병합·공용 mutation gate, state/config XML v1·DPAPI PeerSecret store와 복구 journal, 시스템 파일 로그·보안 진단 경계가 있다. External·Admin·`WDOG` health와 Peer pairing/session/exchange를 평문 HTTP `HttpListener`에 연결했고 승인 대기·승인·거절과 Tray UI를 구현했다. 인증서 전환 작업으로 사이트 CA primitive, DPAPI CA key·canonical CA metadata/full ledger/signed CRL 저장, 암호화 backup, CA 상태·원장·serial 폐기 Admin API와 repair restore 진입점 소스를 추가했다. 이는 부분 구현이며 HTTPS binding, TOFU·pin, 실제 등록·갱신 발급, 등록 모드와 pending 제거는 후속 구현 대상이다. 2026-07-20 locked restore·`Release|x64` 빌드·568개 테스트·package와 설치 EXE 생성은 성공했으며 실제 서비스·설치와 Milestone 환경 검증은 완료되지 않았다.
...
| 항목 | 결정 | 근거·주의 |
|---|---|---|
| 제품·빌드 버전 | 현재 `v1.0.0 build 12` | 단일 원본은 루트 `VERSION`. 이후 버전과 build 번호 변경은 루트 `AGENTS.md` §12를 따르며 일반 코드·계획 수정이나 빌드 체크·커밋·푸시만으로 변경하지 않음 |
...
| 영역 | 확정 결정 | 현재 구현 상태 |
|---|---|---|
| 프로젝트 기반 | .NET Framework 4.8 x64용 `MSTest.TestFramework`·`MSTest.TestAdapter` exact `4.3.2`, PackageReference lock, `tools/test.ps1`와 `tools/package.ps1` 표준 명령 계약 | 테스트 프로젝트·lock·test/package 진입점과 Inno Setup source 구현. locked restore·`Release|x64` build·568개 test·package 성공 |
...
| 프로젝트·파일 | 현재 책임 | 검증 상태 |
|---|---|---|
| `DEEPAi.ServiceDirectory.Domain` | ProductCode·서비스 입력 검증, immutable snapshot·활성 조회, 등록·승인·거절·삭제 순수 상태 전이, logical clock 발급·관찰과 결정적 revision 비교·순수 sync snapshot·병합 primitive, 분리된 Directory/service DNS+IPv4 endpoint identity와 certificate ledger 상태 | endpoint identity·ledger 불변식 소스 추가. 현재 변경분을 포함한 `Release|x64` 컴파일과 자동 테스트 성공 |
...
| `DEEPAi.ServiceDirectory.ExternalProtocol` | 44자 일일 API 키, exactly-one header·ProductCode 검증, External DTO·strict XML codec와 세 endpoint core handler | 메인 Service `HttpListener` composition에 연결되고 `Release|x64` 빌드·계약 테스트 성공. 실제 HTTP 실행 미검증 |
| `DEEPAi.ServiceDirectory.Infrastructure` | 저장 XML·config/state/DPAPI peer store·복구 journal, 로그·보안 진단, External·Admin·`WDOG` HTTP handler, Admin application handler, Peer pairing/session·인증 HTTP exchange·scheduler, 공용 native DLL 검색 정책, PKI core | CA·Directory/service leaf·CSR·serial·CRL primitive와 DPAPI CA key·metadata·ledger·CRL 저장, backup·상태/원장/폐기 handler/host 연결 소스 추가. `Release|x64` 빌드·자동 테스트 성공, 실제 DPAPI/ACL·HTTP 실행 미검증 |
| `DEEPAi.ServiceDirectory.InternalProtocol` | embedded `admin.xsd`·`peer.xsd` 기반 Admin·Peer strict DTO/XML codec, canonical Peer endpoint와 Exchange·pairing·handshake·release·revoke wire 모델 | 실제 Admin/Peer handler에서 사용하고 `Release|x64` 빌드·codec/HTTP 계약 테스트 성공. 실제 network 통합 미검증 |
| `DEEPAi.ServiceDirectory.Service` | x64 Windows Service composition, 설치 identity 검증, 공용 `HttpListener`, endpoint별 인증·deadline·drain, runtime fatal 전파 | 소스와 solution wiring 및 `Release|x64` 빌드 완료. SCM·listener·Negotiate/NTLM·중지/장애 실행 검증 미실행 |
...
| `Directory.Build.props/targets`, `tools/*.ps1`, `installer`, `tests` | 루트 `VERSION` metadata, Release PDB 정책, x64 build/test/package 진입점, Inno 설치 흐름과 계약 테스트 소스 | build 11 Windows Server 2016 최초 설치 실패를 PowerShell 5.1 generic list 반환 문제로 재현·수정. `Debug|x64`, `Release|x64`, 568개 test, ACL round-trip과 build 12 package 성공. build 12 실제 재설치 미검증 |

메인 Windows Service, 실제 `HttpListener` host, Admin handler, Peer 상태/session/DPAPI·sync scheduler, config·PeerSecret 복합 저장과 Inno Setup 소스는 현재 작업 트리에 추가됐고 Release 빌드·568개 테스트·package가 성공했다. Windows 서비스 실행·실제 설치 검증 전에는 제품 기능 전체를 운영 완료로 세지 않는다.
...
현재 제품 runtime은 변경 전 HTTP·pending 기준선이다. 인증서 전환 단계 1의 일부 primitive와 단위 테스트 소스만 추가됐고 단계 1 종료 조건과 단계 2~5는 미완료다. 현재 Release 빌드·568개 테스트·package 성공은 자동 검증 결과이며 HTTPS·PKI 실제 실행이나 현장 검증 완료로 확대 해석하지 않는다.
...
- [ ] 실제 Windows 서비스 실행 검증 — 현재 작업 트리 locked restore·Release build·568개 codec/HTTP/와치독 포함 자동 테스트는 성공
