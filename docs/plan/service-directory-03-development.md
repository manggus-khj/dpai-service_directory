...
> 설계 상태: 사이트 CA·HTTPS·TOFU pin·1시간 1건 등록 모드 목표 계약 확정
> 구현 상태: PKI core 1차 소스와 단위 테스트 소스를 추가했으며 실제 Service는 HTTP·승인 대기 기반이다. 2026-07-20 `Debug|x64` 솔루션 빌드는 경고·오류 없이 성공했으며 테스트 실행·Release·HTTPS·설치·실행 검증은 미완료

Milestone XProtect용 자사 소프트웨어의 접속 경로와 사이트 인증서 신뢰를 관리하는 서비스 디렉토리의 제품 범위와 구현 계획을 정의한다. 인증서 전환의 차이·구현 순서는 [인증서 전환 변경계획](./service-directory-02-certificate-transition.md), 요청·응답의 상세 계약은 [API 명세 안내](./service-directory-04-api.md)와 분리된 내·외부 명세를 따른다.
...
4. 인증서 자동 갱신·폐기·CA backup/restore/rotation
5. 두 서비스 디렉토리 인스턴스 사이의 데이터 동기화
6. 서비스 상태 감시와 제한된 로컬 서비스 제어

현재 저장소에는 x64 솔루션과 논리 버전 snapshot·결정적 sync 병합·공용 mutation gate, state/config XML v1·DPAPI PeerSecret store와 복구 journal, 시스템 파일 로그·보안 진단 경계가 있다. External·Admin·`WDOG` health와 Peer pairing/session/exchange를 평문 HTTP `HttpListener`에 연결했고 승인 대기·승인·거절과 Tray UI를 구현했다. 인증서 전환 작업으로 사이트 CA primitive, DPAPI CA key·canonical CA metadata/full ledger/signed CRL 저장, 암호화 backup, CA 상태·원장·serial 폐기 Admin API와 repair restore 진입점 소스를 추가했다. 이는 부분 구현이며 HTTPS binding, TOFU·pin, 실제 등록·갱신 발급, 등록 모드와 pending 제거는 후속 구현 대상이다. 2026-07-20 restore와 `Debug|x64` 빌드는 성공했으며 locked restore·Release·실행·설치·테스트·package와 Milestone 환경 검증은 완료되지 않았다.
...
| 항목 | 결정 | 근거·주의 |
|---|---|---|
| 제품·빌드 버전 | 현재 `v1.0.0 build 8` | 단일 원본은 루트 `VERSION`. 이후 버전과 build 번호 변경은 루트 `AGENTS.md` §12를 따르며 일반 코드·계획 수정이나 빌드 체크·커밋·푸시만으로 변경하지 않음 |
...
| IP 지원 범위 | IPv4 only | Directory·External service·Peer·installer·firewall·certificate IP SAN 전체에서 IPv6를 거부. 현재 v1의 IPv6 입력 허용은 목표 migration에서 제거 |
...
| 영역 | 확정 결정 | 현재 구현 상태 |
|---|---|---|
| 프로젝트 기반 | .NET Framework 4.8 x64용 `MSTest.TestFramework`·`MSTest.TestAdapter` exact `4.3.2`, PackageReference lock, `tools/test.ps1`와 `tools/package.ps1` 표준 명령 계약 | 테스트 프로젝트·초기 lock·test/package 진입점과 Inno Setup source 구현. restore와 `Debug|x64` build 성공; locked restore·test·Release·package 미실행 |
| 인증서 구현 라이브러리 | `BouncyCastle.Cryptography` exact `2.6.2` | .NET Framework 4.8에 PKCS#10 CSR·X.509 발급·CRL 생성 API가 없어 Infrastructure와 테스트에서 사용. net461 asset, package dependency 없음, MIT 고지·lock 반영. PKI core와 테스트 소스의 `Debug|x64` 컴파일 성공, 테스트 실행 미완료 |
| leaf CRL Distribution Point | 한 fullName의 DNS·IPv4 absolute HTTPS URI 두 개 | URI형 GeneralName에 상대 `/pki/crl`을 넣지 않고 `https://{DirectoryHostName}:21000/pki/crl`과 `https://{DirectoryIpv4Address}:21000/pki/crl`을 발급 leaf에 넣는다. 두 URI는 같은 signed DER CRL을 반환하며 API envelope의 relative `CrlUri`와 구분 |
| 저장 복구 | 현재 XML v1·journal 기준선을 CA·ledger·CRL transaction과 새 schema로 migration | CA metadata·full ledger·signed CRL·CA certificate·DPAPI key fixed target과 CA 폐기·backup marker crash consistency는 소스로 반영. pending 제거와 directory 등록 transaction의 PKI 결합은 미구현 |
...
| 프로젝트·파일 | 현재 책임 | 검증 상태 |
|---|---|---|
| `DEEPAi.ServiceDirectory.Domain` | ProductCode·서비스 입력 검증, immutable snapshot·활성 조회, 등록·승인·거절·삭제 순수 상태 전이, logical clock 발급·관찰과 결정적 revision 비교·순수 sync snapshot·병합 primitive, 분리된 Directory/service DNS+IPv4 endpoint identity와 certificate ledger 상태 | endpoint identity·ledger 불변식 소스 추가. 현재 변경분을 포함한 `Debug|x64` 컴파일은 성공했으며 단위 테스트 실행은 미완료 |
| `DEEPAi.ServiceDirectory.Application` | snapshot load·commit 결과와 저장 계약, 공용 mutation gate, commit 성공 뒤 snapshot 게시와 recovery 차단·재적재, 승인 서비스 조회, 다중 inbound sync batch staging | 소스 결합 및 `Debug|x64` 빌드 완료. 테스트 실행과 강제 종료 검증 미완료 |
| `DEEPAi.ServiceDirectory.ExternalProtocol` | 44자 일일 API 키, exactly-one header·ProductCode 검증, External DTO·strict XML codec와 세 endpoint core handler | 메인 Service `HttpListener` composition에 연결되고 `Debug|x64` 빌드 성공. HTTP 실행·계약 테스트 미검증 |
| `DEEPAi.ServiceDirectory.Infrastructure` | 저장 XML·config/state/DPAPI peer store·복구 journal, 로그·보안 진단, External·Admin·`WDOG` HTTP handler, Admin application handler, Peer pairing/session·인증 HTTP exchange·scheduler, 공용 native DLL 검색 정책, PKI core | CA·Directory/service leaf·CSR·serial·CRL primitive와 DPAPI CA key·metadata·ledger·CRL 저장, backup·상태/원장/폐기 handler/host 연결 소스 추가. `Debug|x64` 빌드 성공, 실행·테스트 미검증 |
| `DEEPAi.ServiceDirectory.InternalProtocol` | embedded `admin.xsd`·`peer.xsd` 기반 Admin·Peer strict DTO/XML codec, canonical Peer endpoint와 Exchange·pairing·handshake·release·revoke wire 모델 | 실제 Admin/Peer handler에서 사용하고 `Debug|x64` 빌드 성공. codec/HTTP 계약 테스트 미실행 |
...
| `DEEPAi.ServiceDirectory.Tray` | WPF 라이트 테마 상태 모니터, 일반 10pt, 기본 `800x700`·최대 `800x720` 창, 제공 PNG tray icon, loopback Negotiate Admin client, 제한된 와치독 Named Pipe client, 페이지·429 backoff·polling과 관리 명령 UI | 설정 화면에 CA 상태·암호 확인을 거치는 암호화 backup·인증서 원장·serial 폐기를 Admin API로 연결하고 `Debug|x64` 빌드 성공. 실행·DPI·접근성·실제 통합 미검증 |
| `DEEPAi.ServiceDirectory.Watchdog` | 별도 x64 Windows Service, `WDOG` health 10초/전체 3초 deadline, 3회 실패·10분 3회 restart latch, 제한 ServiceController, 보호 Named Pipe server | 소스·계약 테스트와 설치 helper 구성 추가 및 `Debug|x64` 빌드 성공. 서비스 설치·SCM/pipe·AD/Workgroup 실행 검증 미실행 |
| `Directory.Build.props/targets`, `tools/*.ps1`, `installer`, `tests/DEEPAi.ServiceDirectory.Tests` | 루트 `VERSION` metadata, Release PDB 정책, x64 build/test/package 진입점, Inno 설치 흐름과 계약 테스트 소스 | 2026-07-20 restore와 현재 작업 트리 `Debug|x64` 빌드 성공. locked restore·Release·test·package·설치 실행 미검증 |

메인 Windows Service, 실제 `HttpListener` host, Admin handler, Peer 상태/session/DPAPI·sync scheduler, config·PeerSecret 복합 저장과 Inno Setup 소스는 현재 작업 트리에 추가됐고 `Debug|x64` 빌드가 성공했다. 테스트·package·Windows 실행·실제 설치 검증 전에는 제품 기능 전체를 운영 완료로 세지 않는다.
...
- 트레이 상태 아이콘의 원본 `tray_icons.psd`와 실제 사용 파일 `tray_running.png`·`tray_stopped.png`는 모두 `docs/plan/service-directory-03-development/`에 둔다. 실제 `RUNNING`에서만 실행 중 아이콘을 사용하고, `STOPPED`·상태 확인 실패에는 중지 아이콘을 사용하면서 tooltip에 정확한 상태 또는 연결 오류를 표시한다.
...
- 선택 주소의 exact HTTPS URL ACL·certificate binding과 loopback URL ACL, 프로그램·포트 방화벽 규칙과 `config.xml`을 함께 구성. 주소·SAN 변경은 installer repair에서 서비스를 멈춘 뒤 검증·leaf 재발급·변경·재기동하며 실패 시 이전 설정으로 롤백
...
| 경로 | 계약의 단일 원본 | 보안 경계 |
|---|---|---|
| 다른 앱 → 메인 | [외부 애플리케이션 API](./service-directory-04-api-01-external-application.md) | Milestone/Directory hostname·IPv4 쌍 기반 HTTPS + site CA TOFU/pin + 일일 키 + 등록 모드·CSR + Domain·Private 방화벽 프로필 |
| 트레이 → 메인 | [내부 API의 Admin](./service-directory-04-api-02-internal.md#4-admin-api) | loopback HTTP + Negotiate Windows identity + 로컬 운영자 그룹 인가 |
| 메인 ↔ 피어 | [내부 API의 동기화](./service-directory-04-api-02-internal.md#5-피어-동기화-데이터) | HTTPS site CA 검증 + ECDH/SAS 페어링 + DPAPI pair root + HMAC-SHA256·freshness·replay 검증 |
| 트레이 → 와치독 | [내부 API의 Named Pipe](./service-directory-04-api-02-internal.md#6-named-pipe-서비스-제어-계약) | 로컬 운영자 그룹 ACL + 명령 허용 목록 |
...
| 파일 | 내용 |
|---|---|
| `directory.xml` | `ServiceHostName`·`ServiceIpv4Address` 필수 쌍을 가진 활성 레코드와 톰스톤, `LogicalClock` high-water mark. 목표 schema는 같은 두 SAN의 active certificate identity를 연계 |
| `pending.xml` | 현재 구현의 승인 대기 파일. 목표 schema migration에서 제거 |
| `config.xml` | IPv4 listener, Directory 접속 identity인 `DirectoryHostName`·`DirectoryIpv4Address`, SiteId·CA mode/rotation, 동기화, `InstanceId`, peer epoch와 로그 보존기간 |
| certificate ledger | serial, ProductCode, issuance request ID/kind, CSR/request payload/SPKI/leaf SHA-256, service SAN identity, issue·expiry·예약 폐기·실제 revoke 상태, CRL high-water와 idempotent 결과. ProductCode별 current 하나, renewal overlap은 retiring, CRL 반영 뒤 revoked |
| signed CRL | site CA가 서명한 canonical DER X.509 CRL |
| CA key/certificate | OS 보호·서비스 SID ACL의 site CA private key와 공개 certificate, canonical CA serial 예약과 Directory HTTPS leaf |
| `secrets/peer.dat` | ECDH 페어링으로 파생한 pair root와 peer identity·key epoch의 DPAPI LocalMachine 보호값 |

현재 `directory.xml`, `pending.xml`, `config.xml`은 `SchemaVersion="1"`이다. 목표 구현은 pending 제거와 PKI state를 위한 명시적 순차 schema migration을 추가한다. 저장 schema는 API version이 아니다. `secrets/peer.dat`과 CA private key는 목적·ACL·backup을 분리한다.
...
| 순서 | 요소 | 저장 규칙 |
|---|---|---|
| 1 | `ListenAddress` | 현재 v1 구현은 설치가 선택한 canonical non-loopback unicast IPv4·IPv6 literal을 저장한다. 목표 migration은 canonical IPv4만 허용하고 기존 IPv6 값이면 listener를 열지 않은 채 명시적 repair를 요구한다. 대괄호·port 없이 저장하며 일반 runtime 설정 commit에서는 불변이다. 주소 변경은 installer repair 전용 commit과 서비스 중지·URL ACL·방화벽·재기동·롤백 흐름에서만 수행한다. |
...
| 2~4 | `PeerEndpoint`, `PeerInstanceId`, `KeyEpoch` | 현재 v1 구현은 canonical `http://{IP literal}:21000`을 저장한다. 목표 migration은 canonical `https://{IPv4}:21000` endpoint와 peer CA identity만 허용하고 IPv6·DNS host endpoint를 거부한다. ID는 소문자 non-empty `D` GUID, epoch는 `1..UInt64.MaxValue`다. |
...
위 형식은 현재 구현 기준선이다. 목표 schema는 `pending.xml`을 제거하고 `DirectoryHostName`·`DirectoryIpv4Address`, SiteId, CA mode·rotation, certificate ledger·CRL high-water를 추가한다. CA 인증서에는 endpoint SAN을 두지 않고 Directory leaf에만 두 Directory identity 값을 각각 DNS·IP SAN으로 넣는다. registration mode state와 ProductCode는 config에 저장하지 않는다. migration 중 필수 current primary나 PKI state가 누락·손상되면 빈 값으로 자동 생성하지 않고 fail closed한다.
...
외부 요청 처리의 상세 계약은 [외부 API 명세 §9](./service-directory-04-api-01-external-application.md#9-즉시-등록과-인증서-발급)에 둔다.
...
최초 페어링은 양쪽 로컬 운영자가 5분 페어링 창을 열고 ECDH P-256 공개키를 교환한 뒤, 각 트레이에 독립적으로 계산된 8자리 SAS가 같은지 양쪽에서 확인하는 상태 머신을 사용한다. 양쪽 확인과 MAC된 commit이 끝나면 `PairedDisabled`가 되고, 운영자가 sync를 활성화해야 `Enabled`가 된다. 상세 계약은 [내부 API 명세](./service-directory-04-api-02-internal.md#53-최초-페어링과-key-epoch)를 따른다.

## 8. 프로젝트 하드닝 적용

[Directory Service 사용 애플리케이션 하드닝 가이드](./service-directory-01-hardening.md)는 공통 보안 기준 위에 적용하는 Directory 구조 제품 전용 추가 기준이다. 저장소 공통 보안 기본 지침은 루트 `AGENTS.md` §8을 따른다.
...
| §2.1·§3.4 시크릿 관리 | 적용 | CA private key PKCS#8과 Peer pair root를 각각 DPAPI `LocalMachine`으로 보호하고 서비스 SID exact ACL로 관리한다. CA backup은 운영자 암호 기반 authenticated encryption을 사용한다. External 일일 키에는 저장 secret이 없음 |
...
| Directory §6 고객 CA | 해당 없음(현재 릴리스) | 고객 하위 CA·외부 서버 인증서 요구가 없으므로 기본 Directory CA mode만 구현. 현장 요구가 생기면 별도 계약·위협 분석 후 재판정 |
...
상세 wire contract와 테스트 벡터의 단일 원본은 [외부 애플리케이션 API §3](./service-directory-04-api-01-external-application.md#3-external-일일-api-키)다.
...
| 1. PKI·저장 | CA·CSR·leaf·ledger·CRL, pending 제거 schema migration과 multi-file crash recovery |
...
| 6. 릴리스 검증 | TOFU·갱신·폐기·CA rotation, 장애·설치·지원 환경과 이중화 종합 테스트 |
...
- 표준 테스트 명령은 `powershell -NoProfile -File .\tools\test.ps1 -Configuration Debug`다. `Debug`와 `Release`만 허용하며 기존 `tools/build.ps1`와 같은 `vswhere`·MSBuild 전제에서 solution `x64` build, 명시적으로 등록된 모든 test assembly의 x64 MSTest 실행과 `artifacts\service-directory\test-results\` TRX 출력을 수행한다. test project 또는 test가 0개이면 nonzero로 실패한다.
...
- 테스트 프로젝트, exact 4.3.2 초기 lock, `tools/test.ps1`, `tools/package.ps1`, Inno Setup `.iss`와 설치 helper 소스는 구현됐다. 2026-07-20 restore와 `Debug|x64` 솔루션 빌드는 경고·오류 없이 성공했다. locked restore, Release build, test와 package는 실행하지 않았고 설치 EXE도 생성하지 않았다. 다음 명시적 검증에서는 locked restore가 현재 프로젝트·package 선언과 일치하는지 실제 명령으로 확인하며, 실행하지 않은 검증을 소스 존재만으로 통과 처리하지 않는다.

현재 제품 runtime은 변경 전 HTTP·pending 기준선이다. 인증서 전환 단계 1의 일부 primitive와 단위 테스트 소스만 추가됐고 단계 1 종료 조건과 단계 2~5는 미완료다. 현재 `Debug|x64` 빌드 성공은 작업 트리의 컴파일 검증이며 HTTPS·PKI 실행이나 테스트 완료로 확대 해석하지 않는다.
...
- [ ] `external.xsd`·`admin.xsd`·`peer.xsd`를 새 CSR·certificate·registration-mode·HTTPS 계약으로 갱신
- [ ] CA·Directory leaf·CSR validator·certificate ledger·CRL core와 고정 vector 테스트 — endpoint identity·CA/leaf·CSR·serial·CRL·ledger 상태, canonical ledger 저장·DPAPI key·backup·serial 폐기 소스는 추가. 고정 vector와 실행 검증은 남음
- [ ] `pending.xml` 제거와 config/directory/ledger/CRL 순차 schema migration·journal 확장
- [ ] remote HTTPS listener, exact certificate binding, OS 보안 기본값의 TLS 1.2+·구버전 비활성 policy 구현
- [ ] `/pki/ca`, `/pki/crl`, `ServiceHostName`·`ServiceIpv4Address` 필수 SAN의 즉시 registration, exact replay와 명시적 service identity 변경 renewal 구현
- [ ] Admin registration-mode 3 endpoint, pending 3 endpoint 제거와 등록 서비스 설정 UI 개편
- [ ] 삭제·재등록의 certificate revoke·CRL publish 원자 transaction과 감사 event 구현
- [ ] Peer HTTPS·동일 site CA·single active issuer와 CA backup/restore/rotation 구현
- [ ] installer ProductCode 입력 부재, CA backup 완료, HTTPS binding repair·rollback·uninstall 보존 구현
...
- [ ] 현재 작업 트리 locked restore·Release build·codec/HTTP/와치독 계약 테스트와 실제 Windows 서비스 실행 검증
