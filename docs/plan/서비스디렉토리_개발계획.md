...
> 구현 상태: PKI core 1차 소스와 단위 테스트 소스를 추가했으며 실제 Service는 HTTP·승인 대기 기반이다. 저장·API·HTTPS·설치·UI 연결과 빌드·실행 검증은 미완료
> 최초 작성일: 2026-07-17
> 최종 정리일: 2026-07-19

Milestone XProtect용 자사 소프트웨어의 접속 경로와 사이트 인증서 신뢰를 관리하는 서비스 디렉토리의 제품 범위와 구현 계획을 정의한다. 인증서 전환의 차이·구현 순서는 [인증서 전환 변경계획](./서비스디렉토리_인증서전환_변경계획.md), 요청·응답의 상세 계약은 [API 명세 안내](./서비스디렉토리_API명세.md)와 분리된 내·외부 명세를 따른다.
...
현재 저장소에는 x64 솔루션과 논리 버전 snapshot·결정적 sync 병합·공용 mutation gate, state/config XML v1·DPAPI PeerSecret store와 복구 journal, 시스템 파일 로그·보안 진단 경계가 있다. External·Admin·`WDOG` health와 Peer pairing/session/exchange를 평문 HTTP `HttpListener`에 연결했고 승인 대기·승인·거절과 Tray UI를 구현했다. 인증서 전환 작업으로 사이트 CA primitive, DPAPI CA key·canonical CA metadata/full ledger/signed CRL 저장, 암호화 backup, CA 상태·원장·serial 폐기 Admin API와 repair restore 진입점 소스를 추가했다. 이는 부분 구현이며 HTTPS binding, TOFU·pin, 실제 등록·갱신 발급, 등록 모드와 pending 제거는 후속 구현 대상이다. 현재 작업 트리의 locked restore·Debug/Release 빌드·실행·설치·테스트·package와 Milestone 환경 검증도 완료되지 않았다.
...
| 항목 | 결정 | 근거·주의 |
|---|---|---|
| 제품·빌드 버전 | 제품 `1.0.0`, 현재 build `6` | 제품 버전은 사용자 명시 요청 때만 변경. 이후 새 변경의 commit+push 전달마다 build를 1 증가하며 같은 전달의 amend·push 재시도에는 중복 증가 금지. 단일 원본은 루트 `VERSION` |
...
| 인증서 구현 라이브러리 | `BouncyCastle.Cryptography` exact `2.6.2` | .NET Framework 4.8에 PKCS#10 CSR·X.509 발급·CRL 생성 API가 없어 Infrastructure와 테스트에서 사용. net461 asset, package dependency 없음, MIT 고지·lock 반영. PKI core 소스는 추가했으나 build·test 미실행 |
...
| 프로젝트·파일 | 현재 책임 | 검증 상태 |
|---|---|---|
| `DEEPAi.ServiceDirectory.Domain` | ProductCode·서비스 입력 검증, immutable snapshot·활성 조회, 등록·승인·거절·삭제 순수 상태 전이, logical clock 발급·관찰과 결정적 revision 비교·순수 sync snapshot·병합 primitive, 분리된 Directory/service DNS+IPv4 endpoint identity와 certificate ledger 상태 | endpoint identity·ledger 불변식 소스 추가. 기존 기반 Debug|x64 컴파일 성공 이력만 있으며 현재 변경분과 단위 테스트는 미실행 |
...
| `DEEPAi.ServiceDirectory.Tray` | WPF 라이트 테마 상태 모니터, 일반 10pt, 기본 `800x700`·최대 `800x720` 창, 제공 PNG tray icon, loopback Negotiate Admin client, 제한된 와치독 Named Pipe client, 페이지·429 backoff·polling과 관리 명령 UI | 설정 화면에 CA 상태·암호 확인을 거치는 암호화 backup·인증서 원장·serial 폐기를 Admin API로 연결. 현재 작업 트리 build·실행·DPI·접근성·실제 통합 미검증 |
...
- 트레이 상태 아이콘의 단일 원본은 `docs/plan/tray_running.png`와 `docs/plan/tray_stopped.png`다. 실제 `RUNNING`에서만 실행 중 아이콘을 사용하고, `STOPPED`·상태 확인 실패에는 중지 아이콘을 사용하면서 tooltip에 정확한 상태 또는 연결 오류를 표시한다.
...
| 경로 | 계약의 단일 원본 | 보안 경계 |
|---|---|---|
| 다른 앱 → 메인 | [외부 애플리케이션 API](./서비스디렉토리_외부애플리케이션_API명세.md) | Milestone/Directory hostname·IPv4 쌍 기반 HTTPS + site CA TOFU/pin + 일일 키 + 등록 모드·CSR + Domain·Private 방화벽 프로필 |
| 트레이 → 메인 | [내부 API의 Admin](./서비스디렉토리_내부_API명세.md#4-admin-api) | loopback HTTP + Negotiate Windows identity + 로컬 운영자 그룹 인가 |
| 메인 ↔ 피어 | [내부 API의 동기화](./서비스디렉토리_내부_API명세.md#5-피어-동기화-데이터) | HTTPS site CA 검증 + ECDH/SAS 페어링 + DPAPI pair root + HMAC-SHA256·freshness·replay 검증 |
...
외부 요청 처리의 상세 계약은 [외부 API 명세 §9](./서비스디렉토리_외부애플리케이션_API명세.md#9-즉시-등록과-인증서-발급)에 둔다.
...
상세 wire contract와 테스트 벡터의 단일 원본은 [외부 애플리케이션 API §3](./서비스디렉토리_외부애플리케이션_API명세.md#3-external-일일-api-키)다.
...
현재 제품 runtime은 변경 전 HTTP·pending 기준선이다. 인증서 전환 단계 1의 일부 primitive와 단위 테스트 소스만 추가됐고 단계 1 종료 조건과 단계 2~5는 미완료다. 기존 Debug|x64 빌드 이력을 현재 HTTPS·PKI 변경분의 검증으로 확대 해석하지 않는다.
