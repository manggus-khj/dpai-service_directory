...
> 구현 상태: 현재 코드는 HTTP·승인 대기 기반 Service·Tray·Watchdog와 저장 복구를 구현하며 인증서 목표 계약은 미구현
> 최초 작성일: 2026-07-17
> 최종 정리일: 2026-07-19
...
| 저장 복구 | 현재 XML v1·journal 기준선을 CA·ledger·CRL transaction과 새 schema로 migration | directory·pending·config·DPAPI PeerSecret store는 현재 구현. pending 제거와 certificate ledger·CRL·CA state의 crash consistency는 미구현 |
...
- 암호화 CA backup의 저장·복원 절차를 설치 완료 조건으로 제공하고 평문 private key export를 금지
...
| `ServiceIpv4Address` | 등록 외부 앱이 자기 서비스 listener용으로 선택·영속화한 canonical dotted-decimal IPv4 한 개. 정확히 4개의 `0..255` 10진수 octet과 `0` 외 선행 0 금지를 적용하고 loopback, `0.0.0.0`, APIPA, multicast, broadcast를 거부한다. IPv6와 IPv4-mapped IPv6를 지원하지 않으며 Directory/Milestone IPv4, TCP source IP와 관찰한 route로 대체하지 않는다. |
| `Port` | `1..65535` 포트 |
...
| certificate ledger | serial, ProductCode, request ID, CSR/SPKI hash, SAN, issue·expiry·revoke 상태, CRL high-water와 idempotent 결과 |
| signed CRL | site CA가 서명한 canonical DER X.509 CRL |
| CA key/certificate | OS 보호·서비스 SID ACL의 site CA private key와 공개 certificate, Directory HTTPS leaf |
...
| §2.1·§3.4 시크릿 관리 | 적용 | CA private key·backup과 Peer pair root를 목적별 OS 보호·제한 ACL로 관리. External 일일 키에는 저장 secret이 없음 |
...
- CSR 자체 서명·key strength와 requested SAN이 등록 요청의 canonical `ServiceHostName` DNS 한 개·`ServiceIpv4Address` IPv4 한 개와 정확히 같은지 검증한다. Directory/Milestone 주소, TCP source IP와 DNS 역조회로 두 값을 만들거나 교정하지 않고 등록·certificate ledger·CRL을 원자 commit한다.
...
현재 제품 소스는 변경 전 HTTP·pending 기준선만 구현한다. 인증서 전환 단계 1~5는 미구현이며 기존 Debug|x64 빌드 이력을 목표 HTTPS·PKI 계약의 검증으로 확대 해석하지 않는다.
...
- [ ] CA·Directory leaf·CSR validator·certificate ledger·CRL core와 고정 vector 테스트
