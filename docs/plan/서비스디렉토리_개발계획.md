...
> 구현 상태: 도메인·저장 복구·External/Admin protocol·테스트 기반과 Tray UI 부분 구현 중, 기존 기반 Debug|x64 빌드 확인·현재 작업 트리 빌드·실행·테스트 미검증
> 최초 작성일: 2026-07-17
> 최종 정리일: 2026-07-18
...
현재 저장소에는 x64 솔루션과 논리 버전 snapshot·결정적 sync 병합·mutation coordinator, state/config XML v1 store와 복구 journal, 시스템 파일 로그·보안 진단 primitive가 있다. External·Admin·`WDOG` health transport-neutral HTTP 경계, strict External·Admin·Peer XML codec, Peer pairing/session 인증·KDF·replay·Push/Pull primitive, Tray UI·client와 별도 Watchdog Windows Service 실행체 및 계약 테스트 소스도 추가되어 있다. 2026-07-18 초기 NuGet restore와 lock을 생성했고 이 추가분 전 기존 기반의 Debug|x64 솔루션 빌드를 경고·오류 없이 확인했지만, 이후 프로젝트 참조가 늘어난 현재 작업 트리의 lock 재생성·빌드·실행·Release·설치·테스트와 Milestone 환경 검증은 완료되지 않았다.
...
| 영역 | 확정 결정 | 현재 구현 상태 |
|---|---|---|
| 프로젝트 기반 | .NET Framework 4.8 x64용 `MSTest.TestFramework`·`MSTest.TestAdapter` exact `4.3.2`, PackageReference lock, `tools/test.ps1`와 `tools/package.ps1` 표준 명령 계약 | 테스트 프로젝트·초기 lock·실제 `tools/test.ps1` 구현. 이후 프로젝트 참조에 대한 lock 재생성·test 실행, `tools/package.ps1`와 Inno Setup source 미구현·미검증 |
| 저장 복구 | 저장 XML 루트 `SchemaVersion="1"`, 명시적 순차 migration, §5.4의 before·after image와 `PREPARED`·`COMMITTED` transaction 저널 | directory·pending serializer·snapshot store, config v1 serializer·config 전용 transaction store, journal engine과 in-process fault test 소스 구현. 현재 작업 트리 빌드·테스트, 공통 mutation gate host 결합, config·PeerSecret 복합 transaction, `.bak` 단독 복구와 실제 process termination 검증 미완료 |
| 운영 | 코드 서명과 설치용 체크섬·manifest를 생성·전달·검증하지 않고 수동 오프라인 설치 EXE를 직접 실행하는 §8.4 승인 예외 | Inno Setup 소스, package 명령과 설치·업그레이드·롤백 검증 미구현 |
...
| `DEEPAi.ServiceDirectory.Application` | snapshot load·commit 결과와 저장 계약, 단일 mutation gate, commit 성공 뒤 snapshot 게시와 recovery 차단·재적재, 승인 서비스 조회, 다중 inbound sync batch staging | 기존 기반 Debug|x64 컴파일 성공. 추가 source의 현재 작업 트리 빌드·테스트와 메인 service host wiring, 로그·sync orchestration은 미검증·미구현 |
| `DEEPAi.ServiceDirectory.ExternalProtocol` | 44자 일일 API 키, exactly-one header·ProductCode 검증, External DTO·strict XML codec와 세 endpoint core handler | 기존 일일 키 기반만 Debug|x64 컴파일 성공. 추가 DTO·codec·handler·계약 테스트는 미검증이며 실제 `HttpListener` host 결합이 남음 |
| `DEEPAi.ServiceDirectory.Infrastructure` | 저장 XML·config/state store·복구 journal, 로그·보안 진단, External·Admin·`WDOG` health transport-neutral HTTP 경계, Peer pairing/session 인증·KDF·replay·inbound 인증·admission coordinator·Push/Pull primitive | 기존 primitive만 Debug|x64 컴파일 성공. 추가 source는 빌드·실행 미검증이며 Event Source·메인 HTTP host, Admin application handler, config·PeerSecret 복합 transaction, `.bak` 단독 복구, Peer 상태·DPAPI·wire orchestration과 재시작 session 무효화가 남음 |
| `DEEPAi.ServiceDirectory.InternalProtocol` | embedded `admin.xsd`·`peer.xsd` 기반 Admin·Peer strict DTO/XML codec, canonical Peer endpoint와 Exchange·pairing·handshake·release·revoke wire 모델 | 현재 작업 트리 빌드·실행 미검증. transport-neutral Admin 경계 외 실제 handler와 Peer HTTP state machine은 없음 |
| `DEEPAi.ServiceDirectory.Tray` | WPF 라이트 테마 상태 모니터, 일반 11pt, 제공 PNG tray icon, loopback Negotiate Admin client, 제한된 와치독 Named Pipe client, 페이지·429 backoff·polling과 관리 명령 UI | 현재 작업 트리 빌드·실행·DPI·접근성 미검증. 메인 HTTP host가 없어 Admin 실제 연결 통합은 아직 불가 |
| `DEEPAi.ServiceDirectory.Watchdog` | 별도 x64 Windows Service, `WDOG` health 10초/전체 3초 deadline, 3회 실패·10분 3회 restart latch, 제한 ServiceController, BOM 없는 UTF-8 한 줄·256바이트·3초 경계와 보호 DACL·client token 이중 검증 Named Pipe server | 소스·계약 테스트 추가. 현재 작업 트리 빌드·서비스 설치·SCM/pipe·AD/Workgroup 실행 검증과 installer의 가상 서비스 계정·서비스 SID 활성화·메인 서비스 제어 DACL 구성 미완료 |
| `Directory.Build.props/targets`, `tools/build.ps1`, `tools/test.ps1`, `tests/DEEPAi.ServiceDirectory.Tests` | 루트 `VERSION` metadata, Release PDB 정책, x64 build와 exact MSTest 4.3.2·TRX 진입점, 도메인·저장·HTTP·Peer·와치독 계약 테스트 소스 | 기존 기반 Debug 빌드만 확인. 추가 프로젝트 참조 뒤 lock 재생성, 현재 Debug·Release build와 test 실행은 미검증이며 package 진입점도 미구현 |

메인 Windows Service와 실제 `HttpListener` host, Admin application handler, Peer 상태/session/DPAPI·sync scheduler, config·PeerSecret 복합 저장 결합과 Inno Setup 설치 프로젝트는 각 계약을 구현할 때 추가한다. 와치독 실행체와 저장·protocol·UI 테스트 소스가 있어도 현재 작업 트리의 빌드·실행 검증 전에는 제품 기능 전체를 구현 완료로 세지 않는다.
...
서비스 runtime은 누락된 `config.xml`을 임의 기본 주소로 자동 생성하지 않고 기동을 중단한다. 설치 초기화만 `LogRetentionDays=30`, `LastPeerKeyEpoch=0`, `State=Unpaired`, `LastResult=NOT_RUN`, notification `NONE`·`NOT_RUN`으로 최초 파일을 만든다. 현재 저장소에는 이 형식의 모델·strict codec·config 단독 journal store와 fault-injection MSTest 소스가 있으나 빌드·테스트와 installer·서비스 composition, 공통 mutation gate 및 config·PeerSecret 복합 transaction 결합은 아직 검증·구현되지 않았다.
...
- 테스트 프로젝트, exact 4.3.2 초기 lock과 `tools/test.ps1`는 구현됐다. 이후 프로젝트 참조가 늘어나 다음 명시적 build check에서는 lock을 먼저 재생성해야 하며 현재 작업 트리의 test 명령은 아직 실행하지 않았다. `tools/package.ps1`와 Inno Setup `.iss`가 없으므로 package 명령은 아직 실행할 수 없으며 TODO·항상 성공하는 placeholder를 만들지 않는다.

단계 0 기반과 단계 1의 state/config 저장·복구 및 MSTest 기반, 단계 2의 External/Admin/Peer protocol·transport-neutral HTTP 경계, 단계 3의 Tray UI와 단계 4의 sync primitive, 단계 5의 Watchdog 실행체 일부를 구현 중이다. 솔루션·프로젝트와 빌드 명령은 실제 파일 기준으로만 문서화하며, 기존 기반에서 확인한 Debug|x64 빌드를 현재 작업 트리의 검증으로 확대 해석하거나 Release·XML 복구·HTTP host·서비스 실행·테스트를 완료로 표시하지 않는다.
...
- [ ] 현재 작업 트리 test 실행, `tools/package.ps1`와 Inno Setup 연계 구현·검증
...
- [ ] 공통 mutation gate와 config·PeerSecret 복합 transaction 결합, `.bak` 단독 복구, migration staging과 실제 process termination·빌드·테스트 검증
...
- [x] Peer pairing·handshake·release·revoke·Exchange strict codec, 인증/KDF/replay·Push/Pull staging primitive 테스트 소스 구현
...
- [ ] 메인 Windows Service·실제 `HttpListener` host/deadline/Negotiate, Admin application handler, Peer 상태/session/DPAPI·sync orchestration, 인증 전 endpoint rate·single-flight와 재시작 session 무효화 구현·검증
- [ ] 현재 작업 트리 lock 재생성·build·codec/HTTP/와치독 계약 테스트와 실제 Windows 서비스 실행 검증
- [x] 코드 서명·설치용 체크섬·manifest 없는 수동 오프라인 설치 예외 확정
- [ ] Inno Setup `.iss`, package 단일 EXE 출력과 수동 설치·업그레이드·롤백 구현·검증
