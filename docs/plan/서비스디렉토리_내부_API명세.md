...
> 문서 상태: 내부 wire 계약 확정
> 구현 상태: Admin 12개 endpoint와 `WDOG` loopback health, strict Admin·Peer XML codec, 메인 Windows Service의 실제 `HttpListener`·Negotiate 선택·deadline·drain, Admin application handler, Peer pairing/session/DPAPI·인증 HTTP exchange·scheduler 및 Watchdog Windows Service·Named Pipe 소스 구현. 현재 작업 트리의 locked restore·빌드·테스트·실제 Windows 실행·설치는 미검증
