...
- 포트: TCP `21000`
- 프로토콜 의미: HTTP/1.1
- API URL, XML payload와 협상에는 별도 API·protocol version 필드를 두지 않는다. 현재 무버전 경로와 기존 필드 의미를 계속 유지하며 호환 확장은 상세 명세가 미리 정의한 선택적 확장점 또는 별도 endpoint로만 수행한다. 알 수 없는 XML 요소 처리 정책이 확정되기 전에는 임의의 선택 필드를 추가하지 않는다. 암호 목적 label의 `v1`은 API 버전이 아니라 고정 domain-separation 문자열이다.
...
계약 확정 전에 논리 오류를 표준 `400`·`404`·`409`로 옮길지 외부 계약과 함께 결정한다. 클라이언트는 성공 HTTP 상태에서도 envelope `Code`를 확인한다.
...
- 새 `KeyEpoch` 후보는 `max(initiator LastPeerKeyEpoch, responder LastPeerKeyEpoch) + 1`이다. 어느 쪽 값이 unsigned 64-bit 최댓값이면 페어링을 거부한다. transcript의 논리 필드 집합은 `PairingId`, 양쪽 nonce, 양쪽 정규화된 `InstanceId`, 양쪽 명시적 endpoint, 양쪽 공개키, 양쪽 `LastPeerKeyEpoch`, 새 `KeyEpoch`, 알고리즘 식별자 `DPAI-SD-ECDH-P256-HMAC-SHA256-v1`이며 양쪽 값이 있는 필드는 initiator 값을 responder 값보다 앞에 둔다. 각 필드는 4바이트 big-endian 길이로 구분하고 전체를 `SHA-256`으로 해시한다. 다만 필드 그룹 사이의 정확한 순서와 GUID·epoch를 포함한 필드별 raw/UTF-8 표현은 아직 확정되지 않았으므로 transcript builder와 wire handler 구현을 차단한다.
...
- 로컬·원격 확인 결정은 `POST /api/sync/pairing/decision`으로 교환하고 두 확인을 모두 검증한 뒤에만 `BothConfirmed`로 전이한다. key-confirm용 pairing confirmation key를 재사용하지 않으며 별도의 방향별 decision 목적 key를 사용한다. 목적 label, canonical MAC 입력과 freshness·재전송 방지 값은 아직 미확정이므로 이 endpoint의 wire handler 구현을 차단한다.
- 양쪽 SAS 확인이 끝나면 pair root와 `PairingId`, transcript hash, peer binding, 새 key epoch를 DPAPI로 보호한 `PairedPendingCommit` 레코드로 먼저 원자 저장하고 `config.xml`의 `LastPeerKeyEpoch`도 같은 복구 작업에서 해당 epoch로 증가시킨다. 이 시점 뒤 실패·취소가 나도 epoch를 되돌리거나 재사용하지 않는다. 그 뒤 `POST /api/sync/pairing/commit`을 pair root에서 별도 파생한 방향별 commit 목적 key로 인증한 멱등 요청·응답으로 교환한다. 목적 label, canonical MAC 입력과 freshness·재전송 방지 값은 아직 미확정이므로 wire handler를 구현하지 않는다. 양쪽 commit 확인을 저장한 뒤에만 `PairedDisabled`로 전이하며 어느 단계에서도 sync를 허용하지 않는다.
...
- Pairing transcript 필드별 정확한 바이트 인코딩·순서
- Pairing decision/commit의 별도 목적 key label, canonical MAC과 freshness·재전송 방지 입력
- 내부·외부 API의 최종 HTTP 상태와 envelope 오류 매핑
- 버전을 포함하지 않는 고정 XML namespace·XSD와 알 수 없는 요소 처리
- `LogicalClock` high-water를 포함하는 다중 파일 journal·backup 복구 형식
