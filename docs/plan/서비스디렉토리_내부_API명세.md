...
> 문서 상태: 내부 wire 계약 확정
> 구현 상태: bounded strict UTF-8 XML 입력, Named Pipe wire codec, listener endpoint guard, Admin 로컬 운영자 SID 인가, 보안 진단 Event Log·flood limiter와 Peer P-256 pairing 암호 primitive 부분 구현, Admin·Peer·Pipe 실행체와 host 통합 미구현·미검증
...
- Admin 속도 제한은 인증된 identity와 서비스 인스턴스를 기준으로 이동 1분 창에서 읽기 `60회/분`·burst `15`, 변경 `10회/분`, `/admin/sync/now` `2회/분`이다. 동시에 처리하는 Admin 요청은 서비스 인스턴스당 최대 8개다.
...
- Admin 인증 실패는 `4102`, 운영자 그룹 인가 실패는 `4103`, Peer HMAC·identity·epoch·freshness·replay·session 실패는 `4104`, 연결 뒤 Named Pipe client token 인가 실패는 `4105`, HTTP local/remote endpoint 경계 위반은 `4106`을 사용한다.
...
4. key epoch
5. session ID. handshake·revoke 요청은 길이 0
6. 대문자 HTTP method
7. 정규화된 path와 query
...
4. key epoch
5. 발급되었거나 요청에서 검증한 session ID. revoke 응답은 길이 0
...
Pull 요청은 마지막 Push ACK의 `ServerSnapshotId`와 0부터 연속 증가하는 batch index를 사용한다.
...
Pairing transcript·decision·commit, HTTP 상태, 고정 namespace와 XSD·확장 정책은 이 문서에서 확정했다. HTTP host, DTO·serializer, pairing state/session 저장과 wire handler는 아직 구현·실행 검증하지 않았다.

API wire 밖의 남은 선행 설계는 `LogicalClock` high-water를 포함하는 다중 파일 journal·backup 복구 형식이다. 이 항목이 해결되기 전에는 승인·동기화 commit을 내구적 완료로 처리하지 않는다.
