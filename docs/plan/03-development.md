...
> 구현 상태: PKI core와 최초 정식 v1 role별 strict storage를 canonical DNS+IPv4·leaf DER·9개 journal target 형식으로 전환하고 저장·Admin의 pending 경계를 제거했다. registration/renewal exact replay·복합 journal, 등록 모드, 즉시 발급·갱신·삭제 폐기, HTTPS 설치/기동 검증, Peer pinned TLS·PKI state cache와 standby 구성·승격 소스를 연결했다. Admin은 단일 목표 `admin.xsd`의 서비스 DNS+IPv4 pair를 사용한다. Directory IPv4·hostname 두 exact HTTPS prefix 보완까지 포함한 2026-07-21 최신 작업 트리의 `Debug|x64`·`Release|x64` locked restore·빌드와 자동 테스트 663개가 각 구성에서 모두 통과했고 installer ACL round-trip·HTTPS binding rollback·installed-state·live endpoint PowerShell 회귀도 통과했다. 실제 Windows 재설치·DPAPI·TLS·두 장비 role 전환 검증은 남아 있다.
...
4. 인증서 자동 갱신·폐기·CA backup/restore. CA key rotation은 별도 후속 범위
...
| 저장 복구 | 미배포 XML v1·journal 기준선을 CA·ledger·CRL transaction을 포함한 최초 정식 v1로 교체 | role별 PKI strict store, 9개 fixed target, pending 저장 제거와 즉시 등록·재등록·갱신·삭제의 directory·ledger·CRL commit을 연결. 전체 장애 지점 fault injection과 serial 재사용 실행 검증은 남음 |
...
| 6. 릴리스 검증 | TOFU·갱신·폐기·CA backup/restore, 장애·설치·지원 환경과 이중화 종합 테스트. CA key rotation은 별도 후속 계획 |
...
- [x] Peer HTTPS·동일 site CA·single active issuer, CA backup/restore와 standby 구성·명시적 승격 구현. CA key rotation·dual-pin은 별도 후속 계획으로 제외
