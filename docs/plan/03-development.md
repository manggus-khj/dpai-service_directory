...
Milestone XProtect용 자사 소프트웨어의 접속 경로와 사이트 인증서 신뢰를 관리하는 서비스 디렉토리의 제품 범위와 구현 계획을 정의한다. 인증서 전환의 차이·구현 순서는 [인증서 전환 변경계획](./02-certificate-transition.md), 요청·응답의 상세 계약은 [API 명세 안내](./04-api.md)와 분리된 내·외부 명세를 따른다.
