...
| 저장 복구 | 미배포 XML v1·journal 기준선을 CA·ledger·CRL transaction을 포함한 최초 정식 v1로 교체 | CA metadata·full ledger·signed CRL·CA certificate·DPAPI key fixed target과 CA 폐기·backup marker crash consistency는 소스로 반영. pending 제거와 directory 등록 transaction의 PKI 결합은 미구현 |
...
| `pki/state.xml` | SiteId, issuer instance·role, CA serial·SPKI·validity, PKI revision·CRL number와 backup marker |
| `pki/ledger.xml` | active issuer 전용 full ledger. serial, ProductCode, issuance request ID/kind, 발급 당시 Name·port·service SAN, CSR/request payload/SPKI hash, exact replay용 leaf DER, issue·expiry·예약 폐기·실제 revoke 상태 |
| `pki/peer-cache.xml` | standby 전용 공개 PKI cache. 인증된 active issuer의 PKI/CRL high-water와 ProductCode별 current serial·leaf hash·만료를 저장하며 full ledger로 사용하지 않음 |
| signed CRL | site CA가 서명한 canonical DER X.509 CRL |
| CA key/certificate | 공개 site CA certificate는 두 role 모두 보유하고 DPAPI·서비스 SID ACL의 CA private key primary는 active issuer만 보유. standby 구성은 backup key를 Directory leaf 발급 중 메모리에서만 사용. Directory HTTPS leaf/private key는 Windows certificate store·HTTP.sys binding이 소유하고 repair에서 재발급 |
| `secrets/peer.dat` | ECDH 페어링으로 파생한 pair root와 peer identity·key epoch의 DPAPI LocalMachine 보호값 |

현재 소스의 `directory.xml`, `pending.xml`, `config.xml` `SchemaVersion="1"` 형식은 배포되지 않은 개발 기준선이다. 목표 인증서 형식을 최초 정식 `SchemaVersion="1"`로 다시 확정하고 pending 없이 directory·config·role별 PKI state를 생성한다. build 12 이하 개발 데이터는 자동 추론·운영자 매핑·호환 migration 대상으로 읽지 않고 개발·테스트 환경에서 명시적으로 데이터 루트를 초기화한다. 저장 schema는 API version이 아니다. `secrets/peer.dat`과 CA private key는 목적·ACL·backup을 분리한다. 정확한 정식 v1 형식과 교차 파일 불변식은 [저장 schema](./03-development-01-storage-schema.md)가 단일 원본이다.
...
위 형식은 build 12 이하의 미배포 구현 기준선이며 정식 v1 호환 입력이 아니다. 최초 정식 v1은 `pending.xml`을 제거하고 config에 `DirectoryHostName`·`DirectoryIpv4Address`를 추가한다. SiteId·CA role·PKI/CRL high-water는 `pki/state.xml`, active issuer의 발급·idempotency·폐기 이력은 `pki/ledger.xml`, standby가 관찰한 공개 current mapping은 `pki/peer-cache.xml`이 소유하며 rotation 상태는 이번 schema에 없다. CA 인증서에는 endpoint SAN을 두지 않고 Directory leaf에만 두 Directory identity 값을 각각 DNS·IP SAN으로 넣는다. registration mode state와 ProductCode는 config에 저장하지 않는다. 구형 개발 데이터는 명시적 초기화 없이 자동 변환하지 않으며, 정식 v1의 필수 current primary나 PKI state가 누락·손상되면 빈 값으로 자동 생성하지 않고 fail closed한다.
...
아래 `Pending` target을 포함한 allowlist는 build 12 이하 미배포 구현 기준선이다. 최초 정식 v1 journal에서는 `Pending` target을 제거하고 [저장 schema §9](./03-development-01-storage-schema.md#9-recovery-journal-v1)의 9개 fixed target과 operation별 집합을 사용한다. 구형 active journal이 있는 개발 데이터는 자동 폐기하지 않고 데이터 루트 초기화 전 서비스가 중지됐는지 확인한 뒤 개발 환경 전체 초기화 절차로만 제거한다.
...
- build 12 이하의 `journal.xml`은 최대 16KiB의 BOM 없는 strict UTF-8이며 DTD·외부 entity와 알 수 없는 요소·속성을 거부한다. 루트는 `SchemaVersion`, `TransactionId`, `Phase`를 가지며 `Phase`는 exact `PREPARED` 또는 `COMMITTED`다. 이 구형 기준선은 1~4개 `Entry`만 허용하지만 최초 정식 v1은 저장 schema의 canonical 1~9개 target 계약으로 교체한다.
...
- [ ] [저장 schema v1](./03-development-01-storage-schema.md)에 따른 `pending.xml` 제거, config/directory/role별 PKI strict codec·leaf DER ledger·Peer cache·9개 journal target 구현. 미배포 구형 v1 호환 migration은 구현하지 않음
