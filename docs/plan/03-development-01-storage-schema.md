...
> 구현 상태: 최초 정식 v1 strict codec·role별 파일·leaf DER·CA/leaf/CRL/DPAPI 교차 검증, 9개 고정 journal target과 저장 계층의 `pending.xml` 제거를 구현했다. registration/renewal stable payload 고정 vector, request ID exact replay/충돌 판정과 최대 16 KiB leaf를 투영한 16 MiB canonical ledger 사전 거부도 추가했다. operation별 exact target 집합을 한 journal로 commit하고 성공 직후 directory·PKI 메모리 baseline을 함께 갱신하는 복합 저장 경계에 즉시 등록·재등록·renewal의 unique serial·서명·directory/ledger/CRL commit과 exact replay를 연결했다. renewal은 동일 SAN 7일·변경 SAN 24시간 `RETIRING` 예약과 due `Superseded` CRL 게시를 포함한다. 서비스 삭제는 tombstone과 같은 ProductCode의 모든 `CURRENT`·`RETIRING`을 `REVOKED/CESSATION_OF_OPERATION`으로 바꾸고 CRL·high-water와 함께 commit한다. 인증된 active issuer의 더 높은 PKI state는 issuer·revision·CRL signature/hash/number·CURRENT mapping을 검증한 뒤 standby 세 target을 한 journal로 교체한다. repair의 standby 구성은 backup key를 Directory leaf 발급 중에만 사용하고 ledger·key primary를 삭제하며, 승격은 backup high-water·same-revision bytes를 검증하고 metadata·full ledger·cache 삭제·DPAPI key를 원자 전환해 local issuer와 `PkiRevision+1` 및 새 issuer backup을 확정한다. transactional delete는 primary와 기존 `.bak`을 journal discard로 이동해 역할 전환 뒤 금지 backup을 남기지 않는다. 2026-07-21 `Debug|x64`·`Release|x64` 자동 테스트 663개가 모두 통과했으며 실제 전원 장애·DPAPI·디스크 장애 실행 검증은 남아 있다.
