...
이 문서는 서비스 디렉토리 구성요소 사이의 등록 모드·인증서 운영·관리·동기화·서비스 제어 계약을 정의한다. 다른 애플리케이션의 주소 구성, TOFU·pin, CSR 발급·갱신과 대상 서비스 인증서 검증은 [외부 애플리케이션 API 명세](./04-api-01-external-application.md)만을 따른다. role별 full ledger·Peer PKI cache와 원자 저장 형식은 [최초 정식 저장 schema v1](./03-development-01-storage-schema.md)이 소유한다.
...
구현 상태: §4.2~§4.4의 세 route는 process-local owner 한 개에 연결했고 open은 CA `READY`·`ACTIVE_ISSUER`에서만 허용한다. `CLAIMED`의 open·close는 `409`·`1002`, body·query는 `400`으로 거부하며 제거된 pending 조회·승인·거절 route는 body 없는 `404`로 닫는다. 즉시 등록·재등록 발급 완료와 불확실한 실패를 owner에 기록해 마지막 등록 결과로 반환한다. 목표 `admin.xsd` 하나만 embedded schema로 사용하고 pending DTO·codec·legacy schema는 제거했으며 2026-07-21 양 구성 자동 테스트에 포함해 검증했다.

### 4.1 `GET /admin/services`

등록된 서비스 목록을 페이지 단위로 반환한다. 인증서 상태·serial·만료 시각은 이 응답에 결합하지 않고 §4.9의 `GET /admin/certificates`에서 조회한다. `includeDeleted=true`이면 톰스톤을 포함하며 기본값은 `false`다. 요청 형식은 `GET /admin/services?includeDeleted=false&pageSize=100&cursor=...`이고 첫 요청에서는 `cursor`를 생략한다.
...
구현 상태: runtime Admin 삭제는 active issuer·backup 준비 상태에서 directory tombstone, 같은 ProductCode의 `CURRENT`·`RETIRING` ledger 폐기, CRL·`PkiRevision`·`CrlNumber` 증가를 공용 mutation gate의 한 recovery journal로 commit한다. `REGISTERED_SERVICE_DELETED`와 sync 예약은 commit 성공 반환 뒤에만 수행한다. 2026-07-21 양 구성 자동 테스트에서 정상·장애 rollback 경계를 검증했다.
...
- `State`는 `NOT_PROVISIONED`, `BACKUP_REQUIRED`, `READY`다. `NOT_PROVISIONED`는 PKI를 한 번도 만든 적 없는 설치 전환 경계에서만 사용한다. role별 필수 파일 조합이 일부만 존재하거나 backup artifact가 있는데 primary가 없거나 검증되지 않으면 상태 API로 축소해 계속 기동하지 않고 서비스 기동 자체를 fail closed한다.
- `Role`은 `ACTIVE_ISSUER`, `STANDBY`다. `STANDBY`는 서비스·CA 상태 조회와 CRL 제공만 가능하고 full ledger 조회·CA backup·발급·폐기·등록 모드 open을 거부한다. `PkiRevision`·`CrlNumber`는 standby에서 인증되어 내구 저장된 Peer PKI cache의 관찰 high-water다.
- `CaSpkiSha256`은 32바이트 SHA-256의 canonical base64다. CA certificate·private key·내부 경로는 반환하지 않는다.
- active issuer는 최초 승인 backup 완료 전 `BACKUP_REQUIRED`이며 등록 모드 open·발급·폐기를 거부한다. backup 성공과 상태 영속화가 모두 완료된 뒤 `READY`다. standby는 backup marker가 아니라 같은 site CA·Peer cache·signed CRL 검증 성공을 `READY` 조건으로 사용한다.
...
active issuer의 `BACKUP_REQUIRED` 또는 `READY` 상태에서 허용한다. 첫 성공 backup이 `BACKUP_REQUIRED -> READY` 전이 근거다. standby는 공개 cache를 full ledger로 확장하거나 backup으로 내보내지 않고 `409`·`1002 CONFLICT`를 반환한다.
...
active issuer의 full certificate ledger를 serial의 Ordinal 오름차순으로 조회한다. `GET /admin/certificates?pageSize=100&cursor=...` 형식을 사용하며 공통 pageSize·opaque cursor·16 KiB 응답 제한을 적용한다. standby는 Peer의 공개 current mapping을 full ledger로 보이게 만들지 않고 `409`·`1002 CONFLICT`를 반환한다.
...
exchange는 유효한 10분 session ID가 필수다. 발신자의 service snapshot은 활성 레코드와 톰스톤을 포함하고 process-local 등록 모드는 포함하지 않는다. certificate current mapping·revocation과 CRL high-water 동기화는 §5.9의 별도 PKI state 계약을 따른다. 한 service batch는 최대 1,000개이면서 요청·응답 각각 4 MiB 이하여야 한다.
...
- peer는 CRL signature를 site CA로 검증하고 `CrlSha256`·number·issuer와 active certificate mapping을 검증한 뒤 [저장 schema §7.4](./03-development-01-storage-schema.md#74-pkipeer-cachexml)의 standby 전용 공개 cache, metadata와 CRL을 한 recovery transaction으로 교체한다.
- 응답은 4 MiB 이하이며 active certificate는 최대 1,000개다. 전체 ledger history와 CA private key·backup·암호는 전송하지 않으며 standby가 누락 필드를 추정해 full ledger를 합성하지 않는다.
...
CA·leaf·CSR·serial·CRL·DPAPI 저장, 복구 journal, 암호화 backup, CA 운영 Admin API/UI와 installer repair를 연결했다. 단일 목표 `admin.xsd`는 서비스 DNS+IPv4 pair와 registration-mode를 제공하고 pending wire·DTO·codec·legacy schema를 제공하지 않는다. Peer HTTPS·PKI state cache와 backup 기반 standby 구성·승격도 연결했다. 2026-07-21 양 구성 자동 테스트 663개와 installer ACL·HTTPS binding 회귀는 통과했으며 실제 DPAPI·설치·TLS·role 전환 실행은 남아 있다.

- `admin.xsd`: 등록 모드 조회·열기·닫기와 서비스 등록 결과 XSD·DTO·strict codec, 실제 세 route와 process-local owner 연결까지 반영됨
- `peer.xsd`: 서비스 DNS+IPv4와 PKI state ledger·CRL high-water XSD·DTO·strict codec, exchange runtime pair, HTTPS IPv4 endpoint와 pinned TLS trust, session-authenticated PKI state route/storage까지 반영됨
- Admin handler와 설정 UI: pending 3 endpoint·승인 대기 화면 제거, 등록 서비스 화면의 모드·countdown·실제 등록·재등록·실패 결과와 CA 상태·backup·원장·serial 폐기까지 반영됨
- Peer host·client: remote host와 canonical endpoint URL은 HTTPS IPv4이며 outbound는 site CA pin·exact SAN/profile·CRL 검증을 request 전 수행하고 session-authenticated PKI state를 교환함. standby runtime 구성과 실제 양방향 실행은 남음
...
- 설치·운영: 초기 CA backup, Directory leaf/private-key ACL, exact HTTPS binding·rollback, standby 구성·single active issuer 명시적 승격과 기동 전 role별 검증은 반영됨. 실제 설치/TLS·승격 실행 검증은 남음

### 7.2 변경 전 구현 기준선

Admin endpoint·Windows identity·raw route·admission·strict XML·오류 mapping과 실제 메인 `HttpListener` host를 구현하고 양 구성 자동 테스트에 포함해 검증했다. 실제 Windows Negotiate/NTLM·Event Log 실행은 남아 있다.

Pairing transcript·decision·commit, strict header/XML, HMAC KDF·freshness·replay, outbound pairing, DPAPI PeerSecret·config 복합 영속화와 HTTP controller를 구현하고 양 구성 자동 테스트에 포함해 검증했다. 실제 두 장비 페어링·DPAPI·장애 복구 실행은 남아 있다.

일반 sync의 여섯 purpose label·length-prefix KDF, 고정 벡터와 inbound·outbound handshake·session Push/Pull·release·revoke orchestration을 구현하고 양 구성 자동 테스트에서 검증했다. 실제 HTTP 양방향 교환은 남아 있다.
...
API wire 밖의 `LogicalClock` high-water와 ledger·CRL·CA state를 포함하는 다중 파일 journal·backup 복구 형식은 [개발계획 §5.4](./03-development.md#54-schema-migration과-다중-파일-복구-저널)에 따른다. pending target은 제거했고 standalone `.bak`은 자동 복원하지 않는다. 자동 테스트의 fault-injection 경계는 통과했지만 실제 process termination과 양방향 장애 복구 전에는 운영상 내구적 완료로 표시하지 않는다.
