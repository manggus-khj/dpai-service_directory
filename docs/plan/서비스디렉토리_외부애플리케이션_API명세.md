...
> 구현 상태: 미구현. 현재 소스는 평문 HTTP·CSR 없는 승인 대기 계약을 구현하며 이 명세와 일치하지 않음
...
| `ServiceIpv4Address` | 필수 1개. 서비스가 실제 bind하는 canonical dotted-decimal IPv4. loopback·`0.0.0.0`·APIPA·multicast·broadcast·선행 0 금지 |
| `Port` | `1..65535` |
...
- `CrlUri`는 같은 authority의 exact `/pki/crl` relative path다.
...
- RSA 2048 이상 또는 ECDSA P-256 이상
- signature hash SHA-256 이상
...
- CRL Distribution Point는 same authority `/pki/crl`
...
- 새 인증서 수령·검증 전 old leaf를 즉시 폐기하지 않는다. SAN이 같으면 overlap은 최대 7일, SAN이 바뀌면 최대 24시간이며 그 뒤 old serial을 CRL에 추가한다. old certificate 원래 만료가 더 빠르면 그 시각을 넘기지 않는다.
...
현재 소스와 `external.xsd`는 아직 위 목표 계약으로 변경되지 않았다. 후속 구현은 [인증서 전환 변경계획](./서비스디렉토리_인증서전환_변경계획.md)에 따라 XSD·DTO·listener·installer·저장·복구·테스트를 함께 변경해야 한다. 구현 전까지 현재 바이너리가 이 명세를 제공한다고 표시하지 않는다.
