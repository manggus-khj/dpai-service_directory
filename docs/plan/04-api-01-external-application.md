...
> 문서 상태: CA key rotation·dual-pin을 포함한 최초 정식 wire 계약 확정
> 구현 상태: `external.xsd`와 공개 DTO·strict codec에 `TrustBundle`, issuer serial 전용 CRL URI를 반영했고 `STABLE`·`PUBLISHED` live bundle, 두 live issuer CRL 조회, 기존 current alias와 새 leaf의 issuer별 CDP를 runtime에 연결했다. `ACTIVATED` runtime, retired CRL 조회, retiring issuer renewal과 reference client는 후속 구현 중이다. 아래 2026-07-21 빌드·663개 테스트 결과는 이 개정 전 기준선이며 현재 작업 트리의 빌드·테스트 결과가 아니다.
...
| `GET /pki/crl/{CaSerialNumber}` | 서명된 CRL 자체 검증 | exact issuer serial의 live 또는 보존된 DER CRL 수신 |
...
- 계획된 CA key rotation은 현재 유효한 TLS 연결에서 전달된 다음 CA와 pin을 dual-pin 기간에만 수락한다. 상태 전이·revision·issuer별 CRL과 놓친 client의 fail-closed 계약은 [rotation 구현계획](./07-ca-key-rotation.md)을 기준으로 이 명세와 `external.xsd`에 구현 전 확정한다.
  ...
  <Extensions>
    <TrustBundle>
      ...
      <Authority>
        ...
        <CaCertificate>MIIC...</CaCertificate>
        <CaSpkiSha256>base64-sha256-value</CaSpkiSha256>
        <CrlUri>/pki/crl/01A4B5C6D7E8F90123456789ABCDEF01</CrlUri>
        <NotBeforeUtc>2026-07-22T00:00:00Z</NotBeforeUtc>
        <NotAfterUtc>2046-07-22T00:00:00Z</NotAfterUtc>
      </Authority>
    </TrustBundle>
  </Extensions>
</Response>
...
- `TrustBundle`은 새 client의 필수 확장이다. `SiteId`, 양의 `TrustRevision`, `Phase`, phase별 시각과 canonical 순서의 `Authority` 1~2개를 포함한다. 첫 authority는 항상 `CURRENT`다.
- `STABLE`은 `RotationId`·전환 시각 없이 `CURRENT` 하나만 허용한다. `PUBLISHED`는 non-empty `RotationId`, `PublishedUtc`, 그 시각보다 최소 30일 뒤인 `ActivationNotBeforeUtc`와 `CURRENT`·`NEXT`를 허용한다. `ACTIVATED`는 여기에 `ActivatedUtc`, 그 시각보다 최소 30일 뒤인 `RetirementNotBeforeUtc`와 `CURRENT`·`RETIRING`을 허용한다.
- authority 순서는 `CURRENT`가 먼저이고 두 번째는 phase에 따라 `NEXT` 또는 `RETIRING`이다. 각 authority의 CA DER·SPKI·serial·유효기간을 교차 검증하며 `CrlUri`는 exact `/pki/crl/{그 authority의 CaSerialNumber}`여야 한다.
- client는 현재 TLS leaf를 기존 pin으로 먼저 검증한 뒤에만 더 높은 bundle을 채택한다. 낮은 revision과 같은 revision의 다른 canonical bytes를 거부하고 `STABLE -> PUBLISHED -> ACTIVATED -> STABLE` 순서를 벗어난 상태를 저장하지 않는다. bundle·issuer별 CRL cache는 하나의 원자 snapshot으로 교체한다.
- `PUBLISHED`의 `NEXT` pin을 저장하지 못한 client는 `ACTIVATED` TLS 연결에서 새 CA를 자동 신뢰하지 않고 관리자 trust reset을 요구한다. `RETIRING` pin은 더 높은 완료 `STABLE` bundle을 새 `CURRENT`로 검증한 뒤에만 삭제한다.
...
`GET /pki/crl`은 현재 Directory TLS issuer CRL의 호환 alias이며 새 leaf CDP에는 넣지 않는다. 새 leaf와 bundle은 `GET /pki/crl/{CaSerialNumber}`를 사용한다. path serial은 exact 32자리 uppercase hexadecimal이며 query·body를 허용하지 않는다. live `CURRENT`·`NEXT`·`RETIRING`과 보존 중인 `RETIRED` issuer는 HTTP 200과 exact DER을 반환하고 알 수 없거나 보존이 끝난 serial은 HTTP 404·`1001 NOT_FOUND`다. client는 leaf issuer와 같은 authority의 URI만 사용하고 다른 issuer CRL이나 alias로 fallback하지 않는다. CA별 CRL number high-water를 독립 저장한다.
...
- `ACTIVATED`에서는 `RETIRING` CA가 발급한 아직 유효하고 미폐기된 leaf도 proof 인증에 사용할 수 있으며 성공 leaf는 `CURRENT` CA가 발급한다. 앱은 bundle에서 자기 issuer가 `RETIRING`이 된 뒤 30일 안에 갱신을 완료한다.
...
1. chain이 해당 Milestone server identity에 저장한 live bundle의 leaf issuer CA로 끝난다.
2. chain CA의 SPKI pin이 bundle의 같은 issuer pin과 같다.
...
9. leaf의 issuer serial 전용 CDP가 bundle의 같은 authority `CrlUri`와 일치하고 해당 issuer의 CRL number가 저장 high-water보다 낮지 않다.
...
3. 최초 TOFU 또는 저장 pin 검증 뒤 trust bundle을 session마다 확인하고, 장시간 process도 마지막 성공 뒤 24시간 안에 갱신
4. bundle과 issuer별 CA/CRL cache 검증·원자 저장
