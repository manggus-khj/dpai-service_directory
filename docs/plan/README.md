...
> 구현 상태: 기존 HTTP·승인 대기 기반 Service·Tray·Watchdog 소스가 있으며 HTTPS·CA·CRL·등록 모드는 미구현
> 최종 정리일: 2026-07-19
...
문서에서 “확정”은 목표 설계 결정이며 구현 완료가 아니다. 현재 저장소 코드는 여전히 remote HTTP, 일일 API 키, `pending.xml`, approve/reject API와 승인 대기 UI를 사용한다. 목표 문서만 바뀐 상태를 빌드·실행·설치 완료로 표시하지 않는다.

## 목표 운영 기준

- 제품 버전 `1.0.0`, 현재 build `6`. 제품 버전은 사용자 명시 요청 때만 바꾸고 새 commit+push 전달 단위마다 build만 1 증가
...
- 기본 Directory site CA, Directory HTTPS leaf, 서버 앱 CSR 발급, serial ledger와 표준 X.509 CRL 사용. CA certificate 자체에는 endpoint SAN을 넣지 않고 Directory leaf에는 Directory DNS·IPv4, 등록 서비스 leaf에는 해당 서비스 DNS·IPv4 SAN을 각각 모두 포함
